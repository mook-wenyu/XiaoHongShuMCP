...
export function registerBrowserToolsWithPrefix(
	server: McpServer,
	...
	server.registerTool(
		...
		async (input: any) => {
			try {
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify(ok({ dirId, pages: context.pages().length })) },
					],
				};
			} catch (e: any) {
				return {
					content: [
						{
							type: "text",
							text: JSON.stringify(err("CONNECTION_FAILED", String(e?.message || e))),
						},
					],
				};
			}
	...
	server.registerTool(
		...
		async (input: any) => {
			try {
				...
				return {
					content: [{ type: "text", text: JSON.stringify(ok({ dirId: (input as any).dirId })) }],
				};
			} catch (e: any) {
				return {
					content: [
