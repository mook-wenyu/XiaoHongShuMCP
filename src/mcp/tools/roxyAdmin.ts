...
export function registerRoxyAdminTools(
	server: McpServer,
	roxy: IRoxyClient,
	...
	server.registerTool(
		...
		{
			description: "获取空间项目列表（分页）",
			inputSchema: { page_index: z.number().optional(), page_size: z.number().optional() },
		},
		async (input: any) => {
			const { page_index, page_size } = WorkspaceList.parse(getParams(input));
			const res = await policy.use("admin:workspace", async () =>
				roxy.workspaces({ page_index, page_size }),
			);
	...
	server.registerTool(
		...
		async (input: any) => {
			...
			const res = await policy.use(`admin:list:${String(parsed.workspaceId)}`, async () =>
				roxy.listWindows(parsed as any),
			);
			return { content: [{ type: "text", text: JSON.stringify(res) }] };
		},
	);
	...
	server.registerTool(
		...
		async (input: any) => {
			...
			const res = await policy.use(`admin:create:${String(body.workspaceId)}`, async () =>
				roxy.createWindow(body as any),
			);
			return { content: [{ type: "text", text: JSON.stringify(res) }] };
		},
	);
