...
export function registerXhsShortcutsTools(server: McpServer, container: ServiceContainer, manager: RoxyBrowserManager) {
  ...
  // 关键词浏览（轻量滚动，提升可见文本覆盖）
  server.registerTool("xhs_keyword_browse", {
    ...
    try {
      ...
      page = await Pages.ensurePage(context, {});
      const { searchKeyword } = await import("../../domain/xhs/search.js");
      const key = (Array.isArray(keywords) ? keywords : [keywords]).map((s: any) => String(s || "").trim()).filter(Boolean).join(" ");
      const t0 = Date.now();
  ...
  server.registerTool("xhs_select_note", {
    ...
    try {
      const { dirId, keywords, workspaceId } = input as any;
      const keys: string[] = (Array.isArray(keywords) ? keywords : [keywords]).map((s: any) => String(s || "").trim()).filter(Boolean);
      if (!keys.length) return { content: [{ type: "text", text: JSON.stringify(err("INVALID_PARAMS", "keywords 不能为空")) }] };

      const context = await manager.getContext(dirId, { workspaceId });
      page = await Pages.ensurePage(context, {});

      // 1) 进行搜索（用第一个关键词）
      let searchOk: boolean | undefined; let searchVerified: boolean | undefined; let searchMatchedCount: number | undefined; let searchTimeMs: number | undefined;
      try {
        const { searchKeyword } = await import("../../domain/xhs/search.js");
        const t0 = Date.now();
        const r = await searchKeyword(page, keys[0]);
        searchTimeMs = Date.now() - t0;
        searchOk = !!(r as any).ok; searchVerified = !!(r as any).verified; searchMatchedCount = (r as any).matchedCount;
      } catch {}

      // 2) 定位候选锚点并筛选包含任一关键词的卡片
      const regex = new RegExp(keys.map(escapeRegExp).join("|"), "i");
      let target: any | undefined;
      let rounds = 0; let candidateCount: number | undefined; let filteredCount: number | undefined; let filterTimeMs: number | undefined;
      let hrefBefore: string | undefined; let titleBefore: string | undefined; let matchedKeywords: number | undefined;
      ...
      for (let round = 0; round < 4 && !target; round++) {
        ...
        try {
          ...
          try { candidateCount = await anchors.count(); } catch {}
          const filtered = anchors.filter({ hasText: regex });
          const count = await filtered.count();
          filteredCount = count; filterTimeMs = Date.now() - tFilter0;
          if (count > 0) {
            target = filtered.first();
            try { hrefBefore = await target.getAttribute("href") ?? undefined; } catch {}
            try { titleBefore = ((await target.textContent()) || "").trim() || undefined; } catch {}
            try {
              const lower = (titleBefore || "").toLowerCase();
              matchedKeywords = keys.reduce((n, k) => n + (lower.includes(String(k).toLowerCase()) ? 1 : 0), 0);
            } catch {}
          } else {
            // 滚动一段后重试（加载更多卡片）
            try { await scrollHuman(page as any, XHS_CONF.scroll.step); } catch {}
            try { await page.waitForTimeout(XHS_CONF.scroll.shortSearchWaitMs); } catch {}
          }
        } catch {
          // 解析失败也尝试小滚动后重试
          try { await scrollHuman(page as any, Math.floor(XHS_CONF.scroll.step * 0.8)); } catch {}
      ...
      if (target) {
        ...
        try { await hoverHuman(page as any, target); } catch {}
      ...
      const opened = await (async () => {
        try {
          const mask = await resolveLocatorResilient(page as any, XhsSelectors.noteModalMask(), { selectorId: "note-modal-mask-check", verifyTimeoutMs: 800, retryAttempts: 1, skipHealthMonitor: true });
      ...
      try {
        ...
        const m = href.match(/(?:\/explore\/|\/discovery\/item\/|\/search_result\/|\/note\/)([^\/?#]+)/);
        if (m && m[1]) noteId = m[1];
      } catch {}
      ...
      return { content: [{ type: "text", text: JSON.stringify(ok({
        ...
        matchedKeywords,
        metrics: {
          search: { ok: searchOk, verified: searchVerified, matchedCount: searchMatchedCount, timeMs: searchTimeMs },
