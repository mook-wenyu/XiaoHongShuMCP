...
export function registerResourceTools(server: McpServer, container: ServiceContainer, adapter: IAdapter) {
  ...
  server.registerTool("page.snapshot", {
    ...
    try {
      ...
      return { content: [{ type: "text", text: JSON.stringify(ok({ url: page.url(), title: await page.title().catch(() => undefined), a11y: result.tree, stats: { ...(result.stats||{}), clickableCount } })) }] };
    } catch (e: any) { return { content: [{ type: "text", text: JSON.stringify(err("INTERNAL_ERROR", String(e?.message || e))) }] }; }
  });
}
