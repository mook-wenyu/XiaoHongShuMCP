	...
	server.registerTool(
		...
		async (input: any) => {
			...
			try {
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify(mapError(e, "navigate", { dirId, url })) },
					],
				};
			}
		},
				...
				try {
					...
					return {
						content: [
							{
								type: "text",
								text: JSON.stringify(mapError(e, "locator", { dirId, selectorId, target })),
							},
						],
					};
				}
				try {
					...
					return {
						content: [
							{
								type: "text",
								text: JSON.stringify(mapError(e, "action", { dirId, selectorId, target })),
							},
						],
					};
				}
	...
	server.registerTool(
		...
		async (input: any) => {
			...
			try {
				...
				return {
					content: [{ type: "text", text: JSON.stringify(mapError(e, "action", { dirId })) }],
				};
			}
		},
	);
		...
		async (input: any) => {
			...
			try {
				...
				if (enableHuman) {
					const wpm =
						humanObj?.wpm ??
						getProfile(humanObj?.profile ?? container.getHumanizationProfileKey()).wpm;
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify(mapError(e, "action", { dirId, selectorId })) },
					],
				};
			}
		},
			...
			try {
				...
				return {
					content: [
						{
							type: "text",
							text: JSON.stringify(err("SCREENSHOT_FAILED", String(e?.message || e))),
						},
					],
				};
			}
