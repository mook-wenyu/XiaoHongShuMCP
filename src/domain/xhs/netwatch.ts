...
function safeMeta(resp: any): { ok: boolean; url?: string; status?: number } {
	...
	try {
		ok = typeof resp?.ok === "function" ? Boolean(resp.ok()) : true;
	} catch {}
	try {
		url = typeof resp?.url === "function" ? String(resp.url()) : undefined;
	} catch {}
	try {
		status = typeof resp?.status === "function" ? Number(resp.status()) : undefined;
	} catch {}
	return { ok, url, status };
}

export function waitApi<T = any>(
	page: Page,
	matcher: (r: Response) => boolean,
	opts: WaitOpts<T>,
): Waiter<T> {
	...
	const promise = page
		.waitForResponse(predicate, { timeout: opts.timeoutMs })
	...
	return waitApi(page, (r) => r.url().includes("/api/sns/web/v1/search/notes"), {
		...
		map: async (resp) => {
			...
			const items: SearchItem[] = raw
				.map((it: any) => ({
					id: it?.id,
					note_card: { display_title: it?.note_card?.display_title },
				}))
...
export function waitComment(
	page: Page,
	timeoutMs: number,
): Waiter<{ id?: string; note_id?: string; content?: string }> {
