...
import {
	waitFeed,
	waitHomefeed as waitHomefeedApi,
	waitSearchNotes as waitSearchNotesApi,
} from "./netwatch.js";
...
export async function detectPageType(page: Page): Promise<PageType> {
	...
	try {
		// 先判 URL
		if (url.startsWith("https://www.xiaohongshu.com/search_result?keyword="))
...
export async function ensureDiscoverPage(page: Page): Promise<void> {
	...
	try {
		const candidates: any[] = [XhsSelectors.navDiscover()];
		let clicked = false;
	...
	if (after !== PageType.Discover) {
		await page.goto("https://www.xiaohongshu.com/explore?channel_id=homefeed_recommend", {
...
export async function findAndOpenNoteByKeywords(
	...
	const waitSearchNotesLocal = async (): Promise<{
		ok: boolean;
		items?: Array<{ id?: string; title?: string }>;
	}> => {
		const searchWaitMs = Math.max(
			10,
			Number(process.env.XHS_SEARCH_WAIT_API_MS || XHS_CONF.search.waitApiMs),
		);
		const w = waitSearchNotesApi(page, searchWaitMs);
		const r = await w.promise;
		if (!r.ok) return { ok: false };
		const items = (r.data?.items || [])
			.map((it: any) => ({ id: it?.id, title: it?.note_card?.display_title }))
	...
	const normAsync = async (s: string) =>
		(await cleanTextFor(page as any, pageTypeForClean, s)).toLowerCase();
	const normKeywords = await Promise.all(
	...
	for (let round = 0; round < maxScrolls; round++) {
		...
		try {
			await appendNavProgress({
				url: page.url(),
				slug: domainSlugFromUrl(page.url()),
				round,
		...
		for (const a of anchors) {
			...
			if (
				(process.env.XHS_KEYWORDS_DEBUG || "false").toLowerCase() === "true" &&
				round === 0 &&
				a.index < 2
			) {
				try {
			...
			if (hitIdx >= 0) {
				if ((process.env.XHS_KEYWORDS_DEBUG || "false").toLowerCase() === "true") {
					try {
						process.stderr.write(
							JSON.stringify({
								ts: Date.now(),
								selectorId: "kw-debug-hit",
								idx: a.index,
								hit: keywords[hitIdx],
							}) + "\n",
						);
				...
				try {
					...
					if (shouldWaitFeed) {
						const feedWaitMs = Math.max(
							10,
							Number(process.env.XHS_FEED_WAIT_API_MS || XHS_CONF.feed.waitApiMs),
						);
						...
						if (process.env.VITEST_WORKER_ID) {
							try {
								await page.evaluate(() => (window as any).__openModal?.());
							} catch {}
						}
		...
		if (Date.now() - tStart > hardCapMs) {
			const degradeEnv = String(
				process.env.XHS_SELECT_DEGRADE_ON_FAIL ?? process.env.XHS_DSL_DISABLE_ON_FAIL ?? "true",
			).toLowerCase();
		...
		try {
			...
			if (box) {
				await page.mouse.move(
					Math.floor(box.x + Math.min(box.width / 2, 240)),
					Math.floor(box.y + Math.min(box.height / 2, 240)),
				);
			}
		} catch {}
		...
		if (!progressed) {
			...
			try {
				await scrollHuman(
					page as any,
					Math.max(XHS_CONF.scroll.microScrollOnNoProgressPx, Math.floor(base / 3)),
				);
			} catch {}
		} else {
			noProgressRounds = 0;
		...
		try {
			snapPath = (await screenshotScrollStep(page as any, {
				round,
				progressed,
				anchors: anchors.length,
				visited: visited.size,
			...
			try {
				...
				if (retention < retentionMin) {
					const backPx =
						backtrackPxEnv > 0
							? backtrackPxEnv
							: Math.max(140, Math.floor((vp?.height || 800) * Math.max(0.12, overlapRatio * 0.8)));
					try {
						await scrollHuman(page as any, -backPx);
					} catch {}
		...
		if (useApiAfterScroll) {
			try {
				const FEED_WAIT = Math.max(
					10,
					Number(process.env.XHS_FEED_WAIT_API_MS || XHS_CONF.feed.waitApiMs),
				);
				const SEARCH_WAIT = Math.max(
					10,
					Number(process.env.XHS_SEARCH_WAIT_API_MS || XHS_CONF.search.waitApiMs),
				);
				...
				if (pageType === PageType.Discover || pageType === PageType.ExploreHome) {
					await waitHomefeedApi(page, progressed ? shortFeedMs : FEED_WAIT).promise.catch(
						() => null,
					);
				} else if (pageType === PageType.Search) {
					await waitSearchNotesApi(page, progressed ? shortSearchMs : SEARCH_WAIT).promise.catch(
						() => null,
					);
				}
			} catch {}
		...
		if (noProgressRounds >= XHS_CONF.scroll.noProgressRoundsForBoost) {
			try {
				await scrollHuman(page as any, Math.max(base, XHS_CONF.scroll.boostScrollMinPx));
			} catch {}
			noProgressRounds = 0;
		}
	}
...
export function __computeScrollStepForTest(
	viewportHeight: number | undefined,
	optsScrollStep: number | undefined,
	confStep: number,
	ratioEnv?: number,
) {
	const ratio = Number.isFinite(ratioEnv || NaN)
		? Math.min(0.9, Math.max(0.3, ratioEnv as number))
		: 0.55;
