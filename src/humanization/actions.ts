...
import {
	moveMouseCubic,
	hoverHuman as hoverCubic,
	clickHuman as clickCubic,
...
export async function moveMouseTo(page: Page, loc: Locator, opts: MoveOptions = {}) {
	...
	await moveMouseCubic(page, loc, {
		...
		overshootAmount: opts.overshootAmount,
...
export async function hoverHuman(page: Page, loc: Locator) {
	await hoverCubic(page, loc);
}
export async function clickHuman(page: Page, loc: Locator) {
	await clickCubic(page, loc);
}
...
export async function clearInputHuman(page: Page, loc: Locator) {
	// 确保 focus
	try {
		await loc.click({ delay: 30 + Math.random() * 60 });
	} catch {}
	const read = async () => {
		try {
