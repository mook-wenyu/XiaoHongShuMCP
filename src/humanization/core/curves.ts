...
export function makeCurvePath(from: Point, to: Point, opts: CurveOptions = {}): Point[] {
	...
	// 控制点：靠近起终点并添加抖动
	const p1 = {
		x: from.x + dx * 0.3 + (Math.random() * 2 - 1) * distance * 0.1,
		y: from.y + dy * 0.2 + (Math.random() * 2 - 1) * distance * 0.15,
	};
	const p2 = {
		x: from.x + dx * 0.7 + (Math.random() * 2 - 1) * distance * 0.1,
		y: from.y + dy * 0.8 + (Math.random() * 2 - 1) * distance * 0.15,
	};
