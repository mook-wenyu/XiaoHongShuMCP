...
export type TaskFn<T=unknown> = (ctx: BrowserContext, dirId: string, args?: TaskArgs) => Promise<T>;
...
export function listTasks() { return Object.keys(registry).map((name) => ({ name, description: descriptions[name] ?? "", params: params[name] ?? {} })); }
...
const params: Record<string, any> = {
	...
	"xhs.publish": { url: { type: "string" }, images: { type: "string[]" }, title: { type: "string" }, content: { type: "string" }, selectorMap: { type: "object" }, submit: { type: "boolean" } },
