...
export function generateHealthReport(opts: ReportOptions = {}): HealthReport {
  ...
  // 过滤掉样本数不足的选择器
  const validSelectors = allSelectors.filter(
    (s: SelectorHealth) => s.totalCount >= minSampleSize
  );

  const unhealthySelectors = healthMonitor.reportUnhealthy(unhealthyThreshold);

  // 只保留样本数足够的不健康选择器
  const validUnhealthy = unhealthySelectors.filter(
    (s) => s.totalCount >= minSampleSize
  );
 ...
 * @returns 停止函数
 */
export function scheduleHealthReport(
  intervalMs: number,
  opts: ReportOptions = {}
): () => void {
