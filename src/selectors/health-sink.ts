...
async function flushNow() {
  ...
  try {
    ...
    // 回退：简单指数退避并把数据放回缓冲
    buffer.unshift(data);
    backoffMs = Math.min(5000, backoffMs === 0 ? FLUSH_MS : Math.min(5000, Math.floor(backoffMs * 1.7)));
    try { process.stderr.write(`[selector-health-sink] flush error: ${String((e as any)?.message || e)}\n`); } catch {}
  }
}
...
export async function appendNavProgress(rec: { url?: string; slug?: string; round: number; anchors: number; visited: number; progressed: boolean }): Promise<void> {
  await appendHealth({
    ts: Date.now(),
    slug: rec.slug,
    url: rec.url,
