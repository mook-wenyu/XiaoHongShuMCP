...
describe('roxy.windows.list e2e', () => {
  it('lists windows with numeric workspaceId', async () => {
    ...
    const res1 = await client.callTool({ name: 'roxy.windows.list', arguments: { workspaceId: id, page_size: 1 } })
    ...
    const res2 = await client.callTool({ name: 'roxy.windows.list', arguments: {} as any, params: { workspaceId: id, page_size: 1 } as any } as any)
