...
describe("scroll metrics computeRetention", () => {
  ...
  it("falls back to text hash when id missing", () => {
    const prev = [{ index: 0, title: "Hi", text: "Hello World", y: 0 }, { index: 1, title: "X", text: "Y", y: 10 }] as any
    const curr = [{ index: 0, title: "hi", text: "hello  world", y: 5 }, { index: 1, title: "Z", text: "Q", y: 10 }] as any
    const r = computeRetention(prev, curr)
