...
describe("humanization edges", () => {
  it("scroll: segments=1 and 30, micro/macro pauses off/on", () => {
    ...
    const s30 = planScroll(3000, { segments: 30, microPauseChance: 1, microPauseMinMs: 60, microPauseMaxMs: 60, macroPauseEvery: 4, macroPauseMinMs: 150, macroPauseMaxMs: 150 });
  ...
  it("mouse: micro jitter default on; can be disabled via counts/px", () => {
    ...
    const noJitter = planMousePath({ x: 0, y: 0 }, { x: 200, y: 0 }, { microJitterPx: 0, microJitterCount: 0 });
  ...
  it("mouse: overshoot on/off affects tail points", () => {
    ...
    const over = planMousePath({ x: 0, y: 0 }, { x: 100, y: 0 }, { overshoot: true, overshootAmount: 12 });
