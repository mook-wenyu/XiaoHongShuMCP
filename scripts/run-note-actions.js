...
async function main() {
	...
	const { ensureDiscoverPage, closeModalIfOpen, findAndOpenNoteByKeywords } = await import(
		"../dist/domain/xhs/navigation.js"
	);
	const { likeCurrent, collectCurrent, commentCurrent, followAuthor } = await import(
		"../dist/domain/xhs/noteActions.js"
	);
	...
	try {
		...
		const { context } = await manager.getHealthy(dirId, {
		...
		// 2) 打开匹配的笔记
		const r = await findAndOpenNoteByKeywords(page, kwList, {
			maxScrolls: Number(process.env.XHS_SELECT_MAX_SCROLLS || 12),
			settleMs: 200,
